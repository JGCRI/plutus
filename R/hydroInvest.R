...
#' @return Returns data in a form required by gcamInvest.R
#' @export
...
hydroInvest <- function(addition_costs, start_year = 2010){
  ...
  hydro_energy <- hydro_energy %>%
    # Replace "k Hydro" with "Hydro"
    dplyr::mutate(agg_tech = replace(agg_tech, agg_tech == "k Hydro", "Hydro"))
  ...
  for (yr in years) {
    if (yr == years[1]) {
      hydro_energy[yr] <- hydro_energy_orig[yr]
    } else{
      hydro_energy[yr] <- hydro_energy_orig[yr] - hydro_energy_orig[yr - 1]
    } # For first year assign original hydro energy otherwise lag
  }
  ...
  hydro_energy_inc <- hydro_energy %>%
    tidyr::gather(key = "year", value = "value",
                  -Units, -scenario, -region, -agg_tech) %>%
  ...
  # New installed capacity in GW
  hydro_GW_inc <- hydro_energy_inc %>%
    tidyr::gather(key = "year", value = "value",
                  -Units, -scenario, -region, -agg_tech) %>%
    dplyr::mutate(value = value * plutus::assumptions("convEJ2GWh") * (1 / (8760 * plutus::assumptions("hydro_cap_fact"))),
                  Units = "GW")%>%
    tidyr::spread(key = "year", value = "value")%>%
    dplyr::ungroup(); hydro_GW_inc
  addition_costs[['newCap_GW']] <- addition_costs[['newCap_GW']] %>%
    dplyr::filter(agg_tech != 'Hydro')
  addition_costs[['newCap_GW']] <- rbind(addition_costs[['newCap_GW']],
                                         hydro_GW_inc)

  # Cumulative new installed capacity in GW
  ...
  addition_costs[['cumCap_GW']] <- addition_costs[['cumCap_GW']] %>%
    dplyr::filter(agg_tech != 'Hydro')
  addition_costs[['cumCap_GW']] <- rbind(addition_costs[['cumCap_GW']],
                                         hydro_GW_inc_cum)
  ...
  hydro_cost_inc <- hydro_GW_inc %>%
    tidyr::gather(key = "year", value = "value",
                  -Units, -scenario, -region, -agg_tech) %>%
    dplyr::ungroup()%>%
    dplyr::mutate(value = value * plutus::assumptions("hydro_cost_GW") * plutus::assumptions("convUSD_2010_2015"),
                  Units = 'billion 2015 USD',
                  value = dplyr::if_else(value <= 0, 0, value)) %>%
    tidyr::spread(key = "year", value = "value") %>%
    dplyr::ungroup()
  addition_costs[['newCap_cost']] <- addition_costs[['newCap_cost']] %>%
    dplyr::filter(agg_tech != 'Hydro')
  addition_costs[['newCap_cost']] <- rbind(addition_costs[['newCap_cost']],
                                           hydro_cost_inc)

  # Cumulative hydropower costs
  hydro_cost_inc_cum <-  hydro_cost_inc %>%
    ...
    dplyr::ungroup()%>%
    tidyr::spread(key = "year", value = "value")
  addition_costs[['cumCap_cost']] <- addition_costs[['cumCap_cost']] %>%
    dplyr::filter(agg_tech != 'Hydro')
  addition_costs[['cumCap_cost']] <- rbind(addition_costs[['cumCap_cost']],
                                           hydro_cost_inc_cum)
